...
# Edited by Ka-Ping Yee <ping@zesty.ca> 2005-10-21 to accommodate
# a broader range of MySQL configurations.
...
if sys.platform == "win32":
    ...
    def config(what):
        ...
        result = popen("mysql_config --%s" % what).read()
        if result.startswith('Usage: '):
            return []
        return result.strip().replace("'", '').replace('"', '').split()
    ...
    if mysqlclient == "mysqlclient":
        ...
        libs = config("libs_r") or config("libs")
    elif mysqlclient == "mysqld":
        libs = config("embedded") or config("libs")
